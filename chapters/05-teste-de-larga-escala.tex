...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.3\textwidth]{figures/integration_get_data.png}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.3\textwidth]{figures/integration_publish_data.png}
...
Os arquivos de entrada contêm cerca de 500 mil vagas de estacionamento e 1
milhão de viagens, levando em consideração o fluxo de carros da cidade de São
Paulo entre 6h e 10h. Durante a simulação foi feito o registro do tempo em que
foi feito a requisição para a plataforma, o tempo em que foi recebida a
resposta, e se a mesma obteve sucesso ou não. A seguir serão apresentados
resultados preliminares do teste experimental de desempenho realizado acerca do
tempo de resposta (\textit{response time}) e vazão (\textit{throughput}) do
serviço de descoberta de recursos da plataforma.

\textbf{Adicionar aqui um breve texto explicando o ambiente do google cloud que
foi feito o teste. Configuração das máquinas presentes no cluster e etc.}

A carga realizada pelo simulador na plataforma pode ser vista na Figura
\ref{fig:load}, onde podemos ver o número de requisições realizadas em cada
tempo de simulação. É possível perceber que no início da simulação poucos
carros requisitam vagas de estacionamento para a plataforma, já que grande
maioria dos carros ainda estão iniciando ou irão iniciar o seu trajeto. O pico
de requisições acontece no meio da simulação, onde boa parte dos carros já
estão realizando os seus respectivos percursos e chegando em seus destinos, se
mantendo próximo até a finalização de toda a simulação.
...
as requisições que retornaram o código HTTP 200. Várias requisições não foram
respondidas com sucesso pela plataforma, acreditamos que isso tenha acontecido
devido a infraestrutura que utilizamos para a realização do experimento
(\textit{Google Cloud Platform}), onde várias requisições foram bloqueadas.
Outra hipótese é que encontramos um gargalo na plataforma com o que diz
respeito ao serviço de descoberta, novos experimentos serão realizados para a
confirmação ou não dessa hipótese.
...
plataforma responder uma requisição com sucesso e no eixo y a quantidade de
requisições que foram respondidas naquele tempo, representando assim o
\textit{response time} da plataforma. Aproximadamente 71\% das requisições
respondidas com sucesso demoraram até 50 segundos. Entretanto, algumas
requisições gastaram mais de 100 segundos até a resposta com sucesso, essas
representando cerca de 5\% do total. Esses longos tempos de espera até a
resposta da plataforma podem influenciar a experiência dos usuários das
aplicações que as utilizem.
...
A vazão do serviço é apresentada na Figura \ref{fig:throughtput}, ou seja,
quantas requisições foram respondidas com sucesso naquele segundo. Ao
confrontarmos com a Figura \ref{fig:load} podemos verificar que a vazão do
sistema não é condizente com a carga de trabalho aplicada, onde um número menor
de requisições são respondidas com sucesso do que realizadas, o que reflete no
resultado apresentado na Figura \ref{fig:rate}.
...
Consideramos os testes experimentais de desempenho em larga escala realizados a
partir da integração feita ainda não ideais devido a falta de conhecimento a
cerca do ambiente provido pelo serviço de infraestrutura \textit{Google Cloud
Platform}. Estamos estudando e trabalhando para atingirmos resultados mais
satisfatórios.

Contudo, a infraestrutura provida pela integração da plataforma InterSCity e o
simulador InterSCSimulator nos permitiu realizar experimentos de desempenho na
