...
serão expostos ao final, já que a prova de conceito realizada consiste na
integração de ambos.
...
a implantação de um ambiente real com dispositivos de \textit{IoT}. Os
trabalhos apresentados em \cite{galache_2012} \cite{nati_2013}
...
conseguem se aproximar do cenário real de uma metrópole por exemplo, ademais, o
custo e esforço de implementação são altos. Em \cite{mwangama_2013} foram
realizados experimentos sobre a confiabilidade da comunicação M2M
...
configuração de tal ambiente não são baixos.

Algumas pesquisas sobre estratégias de teste para aplicações de \textit{IoT}
também poderiam ser aplicadas a Cidades Inteligentes. \cite{adjih_2015}
...
em consideração que em uma cidade esperamos ter na casa de milhões de sensores,
tal ambiente de teste não pode ser considerado algo realista. Existem trabalhos
...
como \cite{shukla_2017} \cite{shukla_2016}. Esses trabalhos realizam testes
de carga nessas plataformas, mas os mesmos não contemplam peculiaridades de uma
...
Simuladores vêm sendo usando em diversas áreas com o objetivo de simular
diversos cenários complexos, em Cidades Inteligentes isso não é diferente.
Em geral, realizar a implantação de infraestrutras de \textit{IoT} nas cidades
é uma tarefa burocrática e cara, e a simulação se apresenta como uma
ferramenta capaz de contornar esses problemas. Diversos trabalhos de simulações
em cidades vêm sendo desenvolvidos \cite{karnouskos_2009} \cite{kim_2009}
\cite{saha_2004}. Outra vantagem de simuladores é a possível integração de
diferentes cenários, como foi feito em \cite{barba_2012}, onde redes veículares
\textit{ad-hoc} (\textit{VANETs}) foram simuladas juntamente com um sistema de
semáforos inteligentes, e em \cite{waraich_2013}, onde foi apresentada uma
microsimulação de carros elétricos híbridos e \textit{smart grids}.

Neste trabalho visamos desenvolver um método de integração de simuladores e
plataformas de Cidades Inteligentes. Alguns trabalhos envolvendo a integração
de simuladores já foram realizados, como em \cite{wegener_2008}, que acoplou um
simulador de tráfico rodoviário com um de redes, permitindo simulação mais
complexas.
...
\textit{RabbitMQ}, uma implementação do protocolo AMQP, onde o objetivo desse
protocolo é criar um padrão aberto para troca de mensagens assíncronas
interoperável e de larga escala \cite{vinoski_2006}. A principal vantagem dele
é permitir que o \textit{broker} faça as decisões de roteamento, não
necessitando a aplicação ter conhecimento desse processo \cite{vinoski_2006}.
