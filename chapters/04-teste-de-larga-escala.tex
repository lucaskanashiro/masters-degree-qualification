...
técnicas de avaliação utilizada em análises de performance.  Neste trabalho
defendemos que simuladores de Cidades Inteligentes são ferramentas capazes de
gerar cargas realistas para a realização de testes de performance em sistemas e
...
serviços estejam disponíveis e atendam todos os interessado a qualquer momento.
Entretanto, cidades estão inseridas em contextos dinâmicos e imprevisíveis, e
...
inclusive picos e acotecimentos inesperados, os mesmos se apresentam como uma
solução. Com os dados de tais simulações espera-se realizar testes de carga, e
que seja possível a identificaçõe de gargalos nos sistemas previamente,
reduzindo o custo para correção e garantindo que os usuários terão uma boa
experiência utilizando o serviço quando estiver em produção. Dependendo da
implementação e implantação do sistema a ser testado, esses testes de carga
podem se tornar testes de estresse podendo levar a problemas como
\textit{crash} do sistema, nesse caso tal sistema não suportaria a carga gerada
pela cidade em questão, necessitando de uma refatoração visando escalabilidade.
...
performance em sistemas e aplicações de Cidades Inteligentes, foi realizada uma
...
plataforma com o que diz respeito a esse novo cenário. Sendo assim possível
realizar testes de performance na plataforma através do simulador.
...
adicionado a cada uma das viagens em qual vaga iria estacionar o agente ao
chegar no término da viagem, considerando que saberíamos a vaga previamente.
...
simulador recalcula a sua rota para o mesmo chegar na vaga de estacionamento
...
\begin{enumerate}
	...
	\item Agente estaciona na vaga de estacionamento
\end{enumerate}
...
na implementação atual são 10 minutos. Esse é um dos pontos passíveis de
melhoria no decorrer do trabalho, onde é necessário encontrar modelos que
representem melhor a realidade.
...
realização de testes de performance na plataforma. Bem como foi apresentado no
início deste capítulo, existem alguns requisitos que devem ser implementados
para que seja possível a realização de testes de performance na plataforma
...
foi usada a API \textit{Restful} provida pelo mesmo, e o \textit{RabbitMQ} (uma
implementação do protocolo AMQP) foi usado para a atualização do estado das
vagas. O ponto interessante dessa integração é que exercemos carga na
plataforma tanto como uma aplicação que utiliza os serviços providos, como
dispositos de Internet das Coisas que desejam atualizar o estado de recursos da
cidade. As Figuras \ref{fig:descoberta} e \ref{fig:atualizacao} serão
utilizadas para detalhar essa integração de duas vias.
...
simulação, entretanto acabou gerando um gargalo na simulação.  Abaixo será
...
\begin{enumerate}
	...
	para o serviço de descoberta da plataforma e o mesmo retorna a vaga em
	questão. Caso não seja encontrada uma vaga disponível em um raio de 500
	metros, esse raio é multiplicado por dois até que se encontre uma vaga
	disponível.

	\item O identificador da vaga é retornado para o \textit{Parking
	Controller}, e o mesmo marca a vaga como utilizada em uma estrutura
	de dados mantida no simulador.

	\item O identificador da vaga é recebido pelo agente que representa o carro
	e o mesmo recalcula a rota para chegar até a mesma.
\end{enumerate}
...
publicação dos eventos da simulação via protocolo AMQP para os interessados,
com isso não foi necesário a utilização de um agente externo, já que a
plataforma também utiliza o mesmo protocolo para divulgação dos seus dados
entre microserviços. A seguir será apresentado mais detalhadamente esse fluxo.
...
\begin{enumerate}
	...
	\textit{Resource Catalog} e \textit{Data Collector}, entretanto o mesmo só
	é utilizado pelo \textit{Data Collector}.
\end{enumerate}
...
Paulo para a realização de testes de performance da plataforma InterSCity.
...
estacionamento, sendo essas recursos da cidade. Os mesmos estão disponíveis
para verificação
...
Na Lista \ref{code:spots} é apresentado um recorte do arquivo de entrada da
...
preliminares do teste de performance realizado acerca do tempo de resposta
...
performance na escala da cidade de São Paulo em um cenário realista, mostrando
