...
comparar quantitativamente diferentes plataformas, entretanto
...
composta de apenas 46 nós. O \textit{FIT IoT-LAB}~\cite{adjih_2015} também é um
ambiente experimental aberto de redes de sensores sem fio para aplicações de
\textit{IoT}, os autores defendem que suportam testes de larga escala,
entretanto o ambiente contem menos de três mil nós na rede. O número de nós
presente na rede de sensores desses \textit{testbeds} não é condizente com o
tamanho de uma grande cidade, onde espera-se milhões de sensores, portanto
inviável para representar ambientes realistas de experimentação.
...
componentes de plataformas para Cidades Inteligentes. O
...
geração de cargas de larga escala. O projeto
...
dados providos pela infraestrutura da cidade, entretanto, não conseguimos
interagir visando atuação. Ou seja, apenas a geração de carga sintética não
permite a realização de testes e experimentos que envolvam sensoriamento e
atuação na cidade, sendo esse tipo de cenário recorrente em Cidades
Inteligentes.

Uma forma para conseguirmos prover esse ambiente de teste e experimentação é a
utilização simuladores, permitindo sistemas e aplicações exercitar tanto o
processamento de dados quanto a atuação na infraestrutura da cidade.
...
sua própria execução, permitindo atuar no cenário simulado de Cidades
Inteligentes. O \textit{TraCI}~\cite{wegener_2008} é uma técnica para
interligar simuladores de tráfego rodoviário e redes veiculares, apesar de não
prover o ambiente de teste e experimentação que desejamos, ele nos apresenta
uma técnica para fazer com que dois simuladores interajam em tempo de
simulação. Tal solução mostra que é possível uma aplicação externa interferir
em uma simulação durante a sua execução, sendo essa a abordagem da solução que
será apresentada nesta pesquisa. Existem vários estudos em que simulações de
diferentes domínios de cidades foram
